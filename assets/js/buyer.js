...
function productCard(p){
  return `<article class="card product-card">
    ...
    <div class="product-body">
      ...
      <div class="row"><b class="price">${money(p.price)}</b><button class="btn" onclick="addCart(${p.id})">افزودن</button></div>
...
function renderCart(){ const box=$('cartBox'); if(!box)return; const rows=cart.map(it=>{const p=findProduct(it.id); return p?`<div class="card pad row wrap"><div><b>${p.name}</b><p class="small">${money(p.price)} × <input class="field" style="width:90px" type="number" value="${it.qty}" onchange="qtyCart(${it.id},this.value)"></p></div><b class="price">${money(p.price*it.qty)}</b><button class="danger" onclick="removeCart(${it.id})">حذف</button></div>`:''}).join(''); const total=cart.reduce((s,it)=>{const p=findProduct(it.id); return s+(p?p.price*it.qty:0)},0); box.innerHTML=(rows||'<p>سبد سفارش خالی است.</p>')+`<div class="card pad"><h3>ثبت اطلاعات مشتری</h3><div class="grid g2"><input id="customerName" class="field" placeholder="نام مشتری"><input id="customerPhone" class="field" placeholder="شماره تماس"><input id="customerCity" class="field" placeholder="شهر/آدرس"><textarea id="customerNote" class="field" placeholder="توضیحات سفارش"></textarea></div><div class="row"><b class="price">جمع: ${money(total)}</b><button class="btn" onclick="submitOrder()">ثبت سفارش</button></div></div>`; }
function submitOrder(){ if(!cart.length)return alert('سبد خالی است'); const name=$('customerName').value.trim(), phone=$('customerPhone').value.trim(); if(!name||!phone)return alert('نام و شماره تماس را وارد کنید'); const order={id:Date.now(), name, phone, city:$('customerCity').value, note:$('customerNote').value, items:cart, total:cart.reduce((s,it)=>{const p=findProduct(it.id);return s+(p?p.price*it.qty:0)},0), status:'در انتظار تماس', createdAt:new Date().toISOString()}; saveOrders([order,...orders()]); cart=[]; saveCart(); $('cartModal').classList.remove('on'); showToast('سفارش ثبت شد'); }
document.addEventListener('DOMContentLoaded',()=>{ mount('buyer'); renderHome(); });
